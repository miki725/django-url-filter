...
class TestFilterSpec(object):
    def test_repr(self):
        ...
        assert repr(
            FilterSpec(["a", "b"], "exact", "value", False, filter_callable=f.foo)
...
class TestLookupConfig(object):
    def test_repr(self):
        ...
        assert repr(config) == ("<LookupConfig foo=>{}>".format(repr({"key": "value"})))
...
class TestSubClassDict(object):
    def test_get(self):
        ...
        mapping = SubClassDict(
            {"a": "b", "z": "b", Foo: "foo", Klass: "klass", "key": "value"}
        )
