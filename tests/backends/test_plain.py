...
DATA = [
    ...
    },
    {
        ...
        "restaurant": {
            "place": 2,
            "waiters": [Bunch(**{"id": 3, "name": "Steve", "restaurant": 2})],
            "serves_hot_dogs": True,
            "serves_pizza": False,
        },
        "name": "Ace Hardware",
...
class TestPlainFilterBackend(object):
    ...
    def test_filter_endswith(self):
        self._test_filter(
            FilterSpec(["restaurant", "waiters", "name"], "endswith", "e", False), DATA
        )
    ...
    def test_filter_exact_simple_list(self):
        self._test_filter(FilterSpec(["nicknames"], "exact", "ace", False), [DATA[1]])

    def test_filter_exact_negated(self):
        self._test_filter(
            FilterSpec(["restaurant", "waiters", "name"], "exact", "John", True), DATA
        )

    def test_filter_gt(self):
        self._test_filter(FilterSpec(["id"], "gt", 1, False), [DATA[1]])

    def test_filter_gte(self):
        self._test_filter(FilterSpec(["id"], "gte", 1, False), DATA)
    ...
    def test_filter_iendswith(self):
        self._test_filter(
            FilterSpec(["restaurant", "waiters", "name"], "iendswith", "E", False), DATA
        )
    ...
    def test_filter_in(self):
        self._test_filter(
            FilterSpec(
                ["restaurant", "waiters", "name"], "in", ["John", "Steve"], False
            ),
            [DATA[1]],
        )
    ...
    def test_filter_iin(self):
        self._test_filter(
            FilterSpec(
                ["restaurant", "waiters", "name"], "iin", ["john", "steve"], False
            ),
            [DATA[1]],
        )

    def test_filter_in_simple_list(self):
        self._test_filter(FilterSpec(["nicknames"], "in", ["ace", "dogs"], False), DATA)

    def test_filter_isnull(self):
        self._test_filter(FilterSpec(["nulldata"], "isnull", True, False), [DATA[1]])
        self._test_filter(FilterSpec(["nulldata"], "isnull", False, False), [DATA[0]])
    ...
    def test_filter_lt(self):
        self._test_filter(FilterSpec(["id"], "lt", 2, False), [DATA[0]])

    def test_filter_lte(self):
        self._test_filter(FilterSpec(["id"], "lte", 2, False), DATA)

    def test_filter_range(self):
        self._test_filter(FilterSpec(["id"], "range", [1, 2], False), DATA)
    ...
    def test_filter_day(self):
        self._test_filter(FilterSpec(["created"], "day", 12, False), DATA)

    def test_filter_hour(self):
        self._test_filter(FilterSpec(["created"], "hour", 9, False), [DATA[0]])

    def test_filter_second(self):
        self._test_filter(FilterSpec(["created"], "second", 37, False), [DATA[1]])

    def test_filter_minute(self):
        self._test_filter(FilterSpec(["created"], "minute", 30, False), DATA)

    def test_filter_month(self):
        self._test_filter(FilterSpec(["created"], "month", 5, False), [DATA[1]])

    def test_filter_year(self):
        self._test_filter(FilterSpec(["created"], "year", 2015, False), [DATA[0]])

    def test_filter_week_day(self):
        self._test_filter(
            FilterSpec(["created"], "week_day", 2, False), [DATA[1]]  # Monday
        )

    def test_filter_exception_handling(self):
        self._test_filter(FilterSpec(["id"], "week_day", 1, False), DATA)
