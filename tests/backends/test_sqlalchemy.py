...
class TestSQLAlchemyFilterBackend(object):
    ...
    def test_filter_already_eagerloaded(self, alchemy_db):
        backend = SQLAlchemyFilterBackend(
            alchemy_db.query(Place).options(
                joinedload(Place.restaurant).joinedload(Restaurant.waiter_set)
            )
        )
        backend.bind(
            [FilterSpec(["restaurant", "waiter_set", "name"], "exact", "John", False)]
    ...
    def test_build_clause_contains(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "contains", "John", Waiter.name.contains("John")
        )

    def test_build_clause_endswith(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "endswith", "John", Waiter.name.endswith("John")
        )

    def test_build_clause_exact(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "exact", "John", Waiter.name == "John"
        )

    def test_build_clause_exact_negated(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "exact", "John", Waiter.name != "John", True
        )

    def test_build_clause_gt(self, alchemy_db):
        self._test_build_clause(alchemy_db, "id", "gt", 1, Waiter.id > 1)

    def test_build_clause_gte(self, alchemy_db):
        self._test_build_clause(alchemy_db, "id", "gte", 1, Waiter.id >= 1)
    ...
    def test_build_clause_icontains_cant_lower(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "icontains", 5, func.lower(Waiter.name).contains(5)
        )
    ...
    def test_build_clause_iexact(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "iexact", "Django", func.lower(Waiter.name) == "django"
        )
    ...
    def test_build_clause_isnull(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "isnull", True, Waiter.name == None  # noqa
        )
        self._test_build_clause(
            alchemy_db, "name", "isnull", False, Waiter.name != None  # noqa
        )
    ...
    def test_build_clause_lt(self, alchemy_db):
        self._test_build_clause(alchemy_db, "id", "lt", 1, Waiter.id < 1)

    def test_build_clause_lte(self, alchemy_db):
        self._test_build_clause(alchemy_db, "id", "lte", 1, Waiter.id <= 1)

    def test_build_clause_range(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "id", "range", [1, 5], Waiter.id.between(1, 5)
        )

    def test_build_clause_startswith(self, alchemy_db):
        self._test_build_clause(
            alchemy_db, "name", "startswith", "Django", Waiter.name.startswith("Django")
        )
